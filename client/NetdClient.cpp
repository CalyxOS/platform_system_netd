...
void randomizeAbstractSockaddr(sockaddr_un* addr, size_t start, socklen_t* addrlen) {
    addr->sun_path[0] = '\0';
...
inline bool userIsSystem(uid_t uid = getuid()) {
    return uid < FIRST_APPLICATION_UID;
}
inline bool isSockaddrAbstract(const sockaddr* addr, socklen_t addrlen) {
    if ((addr != nullptr) && (addr->sa_family == AF_UNIX)) {
        ...
        if (((size_t)addrlen > (sizeof(sa_family_t) + 1)) && (sa_un->sun_path[0] == '\0')) {
            return true;
        }
    }
    return false;
...
void logAbstractSockaddr(const char* str, const sockaddr_un* sa_un, socklen_t addrlen) {
    ssize_t namelen = addrlen - sizeof(sa_family_t);
    char name[namelen];
    memcpy(&name, &sa_un->sun_path[1], namelen-1);
    name[namelen-1] = '\0';
    ALOGE("%s: %u: %s", str, getuid(), name);
}
void logIfAbstractSockaddr(const char* str, const sockaddr* addr, socklen_t addrlen) {
    if (isSockaddrAbstract(addr, addrlen)) {
        logAbstractSockaddr(str, (sockaddr_un*)addr, addrlen);
    }
}
...
int netdClientBind(int sockfd, const sockaddr* addr, socklen_t addrlen) {
    logIfAbstractSockaddr("netdClientBind", addr, addrlen);

    if (!userIsSystem() && isSockaddrAbstract(addr, addrlen)) {
        ...
        if (global_prefix != nullptr) {
            ...
            if (res != 0) {
                randomizeAbstractSockaddr(&sa_un, strlen(global_prefix), &salen);
                logAbstractSockaddr("netdClientBind random", &sa_un, salen);
                return libcBind(sockfd, (sockaddr*)&sa_un, salen);
            } else {
                logAbstractSockaddr("netdClientBind prefix", &sa_un, salen);
                return 0;
            }
        }
...
int callNetdClientGetname(const char* funcname, GetsocknameFunctionType func,
    ...
    struct sockaddr_un sa_un = {
            // Intentionally initialize to unexpected values
            .sun_family = AF_INET,
            .sun_path = "/",
    };
    socklen_t salen = sizeof(sa_un);

    int res = func(sockfd, (sockaddr*)&sa_un, &salen);
    int getnameErrno = errno;

    if (isSockaddrAbstract((sockaddr*)&sa_un, salen)) {
        ...
        if (!userIsSystem()) {
            const char* global_prefix = getAllowedGlobalPrefixIfAny(&sa_un, salen);

            if (global_prefix != nullptr) {
                // No need to randomize, just truncate after the prefix
                size_t len = strlen(global_prefix);
                // Clean anything after the prefix with 1's instead of 0's
                // (Because of the whole "shouldn't be null-terminated" thing)
                memset(&sa_un.sun_path[len], 1, sizeof(sa_un.sun_path) - len);
                salen = sizeof(sa_family_t) + 1 + len;
                logAbstractSockaddr(funcname, &sa_un, salen);
                errno = 0;
                return 0;
            }

            if (reverseIsolateAbstractSockaddr(&sa_un, &salen) != 0) {
                logAbstractSockaddr(funcname, &sa_un, salen);
                errno = -EACCES;
                return -1;
            }
    ...
    memcpy(addr, &sa_un, salen);
    *addrlen = salen;
    return res;
}

int netdClientGetsockname(int sockfd, sockaddr* addr, socklen_t* addrlen) {
    return callNetdClientGetname("netdClientGetsockname", libcGetsockname, sockfd, addr, addrlen);
}

int netdClientGetpeername(int sockfd, sockaddr* addr, socklen_t* addrlen) {
    return callNetdClientGetname("netdClientGetpeername", libcGetpeername, sockfd, addr, addrlen);
}
