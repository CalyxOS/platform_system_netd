...
// in case of errors obtaining the status from libnetd_updatable (Connectivity module)
bool getNetworkingAllowedForProcess() {
    ...
    close(fd);
    // Response is 0 if restricted, 1 if allowed, or a negative POSIX error code on failure
    return networkingAllowedForProcess != 0;
}
