...
int netdClientConnect(int sockfd, const sockaddr* addr, socklen_t addrlen) {
    ...
    if (0 && (addr != nullptr) && (addr->sa_family == AF_UNIX)) {
        sockaddr_un* orig_sa = (sockaddr_un*)addr;
        // Is it an abstract socket? Must have sun_path starting with '\0' but not only '\0'.
...
int netdClientBind(int sockfd, const sockaddr* addr, socklen_t addrlen) {
    // return -1 just to see if anything changes since this is busted
    errno = -EPERM;
    return -1;
