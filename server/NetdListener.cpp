...
NetdListener::NetdListener() : FrameworkListener(SOCKET_NAME) {
    registerCmd(new Handler());
}

NetdListener::Handler::Handler() : NetdCommand("netd") {}
...
int NetdListener::Handler::runCommand(SocketClient* cli, int argc, char** argv) {
    ...
    if (strcmp(argv[1], "traffic") == 0) {
        if (strcmp(argv[2], "getuidnetworking") == 0) {
            cli->sendCode(libnetd_updatable_getNetworkingAllowedForUid(std::stoul(argv[3])));
        }
    }
