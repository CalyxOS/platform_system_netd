...
[[nodiscard]] static int modifyImplicitNetworkRule(unsigned netId, uint32_t table, bool add) {
    // With recent firewall changes, there are already UID-based rules to handle this; adding
    // this implicit rule makes those rules ineffective by allowing any UID on any network.
    if ((true)) return 0;
