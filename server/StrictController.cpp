...
int StrictController::setGlobalCleartextPenalty(StrictPenalty penalty) {
    ...
    if (penalty == ACCEPT) {
        commands.push_back(StringPrintf("-D %s -j %s", LOCAL_OUTPUT, LOCAL_CLEAR_DETECT);
    } else {
...
int StrictController::setUidCleartextPenalty(uid_t uid, StrictPenalty penalty) {
    ...
    if (penalty == ACCEPT) {
        commands.push_back(StringPrintf("-A %s -j %s", perUidChain.c_str(), "ACCEPT"));
    } else if (penalty == LOG) {
        commands.push_back(StringPrintf("-A %s -j %s", perUidChain.c_str(), LOCAL_PENALTY_LOG));
    } else if (penalty == REJECT) {
        commands.push_back(StringPrintf("-A %s -j %s", perUidChain.c_str(), LOCAL_PENALTY_REJECT));
    }
    commands.push_back("COMMIT\n");
