...
int StrictController::setUidCleartextPenalty(uid_t uid, StrictPenalty penalty) {
    ...
    ALOGD("setUidCleartextPenalty(): uid=%d", uid);

    std::string uidOwnerMatch = uid == static_cast<unsigned int>(-1) ? ""
                                            : StringPrintf("-m owner --uid-owner %d", uid);

    ALOGD("setUidCleartextPenalty(): uidOwnerMatch=%s", uidOwnerMatch.c_str());
    std::vector<std::string> commands;
    commands.push_back("*filter");
    if (penalty == INVALID) {
        commands.push_back(StringPrintf("-D %s %s -j %s", LOCAL_OUTPUT, uidOwnerMatch.c_str(),
                                        LOCAL_CLEAR_DETECT));
        commands.push_back(StringPrintf("-D %s %s -j %s", LOCAL_CLEAR_CAUGHT, uidOwnerMatch.c_str(),
                                        perUidChain.c_str()));
        ...
        commands.push_back(StringPrintf("%s %s %s -j %s", operation.c_str(), LOCAL_OUTPUT,
                                        uidOwnerMatch.c_str(), LOCAL_CLEAR_DETECT));
        commands.push_back(StringPrintf("%s %s %s -j %s", operation.c_str(), LOCAL_CLEAR_CAUGHT,
                                        uidOwnerMatch.c_str(), perUidChain.c_str()));
