...
int main() {
    ...
    int fd = retrieveProgram("/sys/fs/bpf/firewall_shared/prog_firewall_cgroupsock_inet_create");
    unique_fd cgroupProg(fd);
    ALOGI("cgroupProg fd %d", fd);
    if (!cgroupProg.ok()) {
        ALOGE("cgroupProg not ok");
        exit(1);
    }
    if (android::bpf::attachProgram(BPF_CGROUP_INET_SOCK_CREATE, cgroupProg, cg_fd, BPF_F_ALLOW_MULTI)) {
        ALOGE("attachProgram failed");
        exit(1);
    }
